...
const LoginVipContainer = ({ ...props }) => {
  const { text, buttonText, buttonBgColor, buttonColor, bgColor } = props;
  return (
    <div className={`${bgColor} container-login-vip`}>
      <p className="text-[1.2rem] font-semibold mb-5">{text}</p>
      <div
        className={`inline-block uppercase font-semibold text-[12px] p-1 rounded-[99rem] border py-[4px] px-[34px] cursor-pointer`}
      >
        {buttonText}
      </div>
    </div>
